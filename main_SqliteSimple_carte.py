...
myFunc = SqliteDb("D:/VsCode_python/1_Projet_OK/Carte_sqliteSimple_pdf_ok/student_blob1.db")
...
#*********************************************************************
    ...
    def insert_student(self):     
        ...
        if text1.strip(" ") != "" and text2.strip(" ") != "" :
            ...
            student =(text1,text2,text3,text4,text5,text6,image_path)
            myFunc.insert("INSERT INTO student_profile(id,fname,lname,date,gender,adress,pathFile) VALUES (?,?,?,?,?,?,?)", student)
    ...
    def fill_From_tableClient_ToTextBox(self):
        ...
        for row in enumerate(resultat):
            if row[0] == self.table_student.currentRow():
                ...
                self.lineEdit_id.setText(str(data[0]))
                self.lineEdit_fname.setText(str(data[1]))
                self.lineEdit_lname.setText(str(data[2]))
                self.lineEdit_date.setText(str(data[3]))
                self.lineEdit_gender.setText(str(data[4]))
                self.lineEdit_adress.setText(str(data[5]))
...
#**********************************************************************
    ...
    def update_student(self):
        ...
        if text2.strip(" ") != "" and text3.strip(" ") != "" :
            
            student =(text2,text3,text4,text5,text6,empPhoto)
            
            myFunc.update("UPDATE student_profile SET fname = ?, lname= ? , date= ? , adress= ?, gender= ?  ,profile_photo= ? WHERE id = "+id_update, student)
            self.refresh()
        else:
            show_message("Error")
...
#        cnn.close()
    ...
    def genererPDF(self): 
        ...
        my_path='my_pdf59.pdf' # file name
        ...
        # choose some colors
            ...
            c.drawString(0.5*inch,1.7*inch,str(row[0])) # id to String
            c.drawString(0.5*inch,1.3*inch,row[1])   # Name  
            c.drawString(0.5*inch,0.9*inch,row[2])    
            c.drawString(0.5*inch,0.5*inch,row[3]) 
            c.drawString(0.5*inch,0.1*inch,row[4])
            c.drawString(0.5*inch,-0.3*inch,row[5])
            my_image=str(row[-1])
